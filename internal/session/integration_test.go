...
func TestIntegration_Ingest_Idempotent_UpsertsSameSession(t *testing.T) {
	...
	err := svc.Ingest(validReq("integ-sess-2"))
	require.NoError(t, err)
	err = svc.Ingest(validReq("integ-sess-2"))
	require.NoError(t, err)

	// Events should accumulate (4 total), session should exist once
	events, err := svc.GetEvents("integ-sess-2")
	require.NoError(t, err)
...
func TestIntegration_FinalizeSession_SetsEndedAt(t *testing.T) {
	...
	require.NoError(t, svc.Ingest(validReq("integ-sess-3")))

	err := svc.FinalizeSession("integ-sess-3")
	require.NoError(t, err)

	sess, err := svc.GetByID("integ-sess-3")
	require.NoError(t, err)
...
func TestIntegration_FailedIngest_SaveAndRetry(t *testing.T) {
	...
	assert.Equal(t, "integ-failed-1", items[0].SessionID)

	err = svc.RetryFailed("fi-1")
	require.NoError(t, err)

	// After retry, session and events should exist
	_, err = svc.GetByID("integ-failed-1")
	require.NoError(t, err)
